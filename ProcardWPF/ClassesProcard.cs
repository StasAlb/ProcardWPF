...
namespace ProcardWPF
{
	...
	public abstract class DesignObject : INotifyPropertyChanged
	{
	    ...
	    protected DeviceType deviceT;

	    public void SetDevice(DeviceType dt)
	    {
	        deviceT = dt;
	    }
    ...
    public class Barcode : DesignObject
    {
        ...
        public override void Draw(DrawingContext dc, Regim regim, bool selected, int step)
        {
            ...
            else
            {
                ...
                // проверяем помещается ли штрих код в отведенное место
                barCode.ImageSettings.ImageFormat = ImageFormat.Bmp;
                try
                {
                    Image im = new Bitmap(new System.IO.MemoryStream(barCode.GetBarcodeImageBinary()));
                    if (im.PhysicalDimension.Width / im.HorizontalResolution > Width ||
                        im.PhysicalDimension.Height / im.VerticalResolution > Height)
                        return;
                }
                catch { return; }
            }
...
//        }
    ...
    public class ImageField : DesignObject
    {
        ...
        public int StyleBinding
        {
            ...
            set
            {
                switch (value)
                {
                    case (int)ImageStyle.FitToField:
                        Style = ImageStyle.FitToField;
                        break;
                    case (int)ImageStyle.AutoSize:
                        Style = ImageStyle.AutoSize;
                        break;
                    case (int)ImageStyle.Background:
                        Style = ImageStyle.Background;
                        break;
                    case (int)ImageStyle.Mask:
                        Style = ImageStyle.Mask;
                        break;
                }
            }
        }
        ...
        public override void Draw(DrawingContext dc, Regim regim, bool selected, int step)
        {
            ...
            if (text.Length > 0)
            {
                ...
                try
                {
                    if (style != ImageStyle.Mask)
                        bi = new BitmapImage(new Uri(text));
                }
                catch
                {
                    return;
                ...
                int tp = 0, lt = 0;
                int rht = 0, rwd = 0;
                Rect drawRect = Rect.Empty;
                switch (style)
                {
                    case ImageStyle.FitToField:
                        #region
                        drawRect = new Rect(Card.ClientXToScreen(X), Card.ClientYToScreen(Y + Height, Side), wd, ht);
                        break;
                        #endregion
                    case ImageStyle.AutoSize:
                        #region

                        if (bi == null)
                            return;
                        double pRatio = (double) wd / (double) ht;
                        double imRatio = bi.Width / bi.Height;
                        if (wd >= bi.Width && ht >= bi.Height)
                        {
                            rwd = Convert.ToInt32(bi.Width);
                            rht = Convert.ToInt32(bi.Height);
                        }
                        else if (wd >= bi.Width && ht <= bi.Height)
                        {
                            rht = (int) ht;
                            rwd = (int) (ht * imRatio);
                        }
                        ...
                        else if (wd <= bi.Width && ht <= bi.Height)
                        {
                            ...
                            {
                                if (bi.Width < bi.Height && imRatio < pRatio)
                                {
                                    rht = (int) ht;
                                    rwd = (int) (ht * imRatio);
                                }
                        ...
                        tp = (int) ((Card.ClientToScreen(Height) - rht) / 2.0);
                        lt = (int) ((Card.ClientToScreen(Width) - rwd) / 2.0);
                        if (tp < 0)
                            tp = 0;
                        if (lt < 0)
                            lt = 0;
                        drawRect = new Rect(Card.ClientXToScreen(X) + lt, Card.ClientYToScreen(Y + Height, Side) + tp,
                            rwd, rht);
                        break;

                    #endregion
                    case ImageStyle.Background:
                        #region
                        x = 0;
                        y = 0;
                        width = Card.Width;
                        ...
                        double ww = 0; 
                        double hh = 0; 
                        if (regim == Regim.ToPrinter && deviceT == DeviceType.SR)
                        {
                            ww = (width * 0.02);
                            hh = (height * 0.02);
                        }
                        wd = Card.ClientToScreen(Width+ww); ht = Card.ClientToScreen(Height+hh);
                        wd = (wd < 0) ? 0 : wd;
                        ht = (ht < 0) ? 0 : ht;
                        drawRect = new Rect(Card.ClientXToScreen(X), Card.ClientYToScreen(Y + Height, Side), wd, ht);
                        break;
                        #endregion
                    case ImageStyle.Mask:
                        #region
                        x = 0;
                        y = 0;
                        width = Card.Width;
                        height = Card.Height;
                        wd = Card.ClientToScreen(Width); ht = Card.ClientToScreen(Height);
                        wd = (wd < 0) ? 0 : wd;
                        ht = (ht < 0) ? 0 : ht;
                        if (regim != Regim.Debug)
                        {
                            dc.PushTransform(new TranslateTransform(Card.ClientXToScreen(X), Card.ClientYToScreen(Y + Height, side)));
                            dc.PushTransform(new ScaleTransform(96.0/300.0, 96.0/300.0) );
                            //dc.Pop();
                        }
                        ...
                        if (File.Exists(text))
                        {
                            using (StreamReader sr = new StreamReader(text))
                            {
                                while (sr.Peek() > 0)
                                {
                                    try
                                    {
                                        dc.DrawGeometry(Brushes.White, null, Geometry.Parse(sr.ReadLine()));
                                    }
                                    catch { }
                                }
                                sr.Close();
                        ...
                        {//dc.PushTransform(new TranslateTransform(Card.ClientXToScreen(X), Card.ClientYToScreen(Y + Height, side)));
                            //dc.PushTransform(new ScaleTransform(300.0 / 96.0, 300.0 / 96.0));
                            dc.Pop();
                        }
                        #endregion
                        break;
    ...
    public class MagStripe : DesignObject
    {
        ...
        public override void Draw(DrawingContext dc, Regim regim, bool selected, int step)
        {
            if (regim == Regim.Design || regim == Regim.Print)
...
//            dc.DrawText(string.Format("~EM%{0};{1:0000};{2:0000};{3}", (int)font, Convert.ToInt32(x * 1000), Convert.ToInt32(y * 1000), text), font, Brushes.Black, 50, 50);
    ...
    public class Card
    {
        ...
        [XmlElement(Type = typeof(Devices.DC450)), XmlElement(Type = typeof(Devices.DC150)), XmlElement(Type = typeof(Devices.Simulator)), 
            XmlElement(Type = typeof(Devices.XPSPrinter)), XmlElement(Type = typeof(Devices.SRPrinter))]
        ...
        public override string ToString()
        {
            ...
            switch (function)
            {
                ...
                case (CompositeFunc.Split):
                    fname = (string)Application.Current.FindResource("Composite_FSplitShort");
                    res = $"{dname} - {fname}[{((parameters.Count > 0) ? parameters[0] : "")},{((parameters.Count > 1) ? parameters[1] : "")}]";
                    break;
                case CompositeFunc.Ean13:
                    fname = (string)Application.Current.FindResource("Composite_FEan13Short");
                    res = $"{dname} - {fname}";
                    break;
                case CompositeFunc.Sphinx:
                    fname = (string)Application.Current.FindResource("Composite_FSphinxShort");
                    res = $"{dname} - {fname}";
                    break;
