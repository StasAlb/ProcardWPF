...
//                return false;
            ...
            { 
                if (card.objects[i].OType == ObjectType.MagStripe)
                {
                    for (int t = 0; t < 3; t++)
                    {
                        ...
                        if (((MagStripe)card.objects[i]).InTypeM[t] == InTypes.Db)
                ...
                if (card.objects[i].InType == InTypes.None)
                    card.objects[i].SetText("");
                if (card.objects[i].InType == InTypes.Db)
        ...
        private string MakeComposite(string composite)
        {
            ...
            for (int i = 0; i < ca.Count; i++)
            {
                ...
                if (ca[i].Type == CompositeType.DB && dataIn != null)
                ...
                try
                {
                    ...
                    if (ca[i].Function == CompositeFunc.SubStringPos && ca[i].Parameters.Count > 0)
                    {
                        int start = 1, len = 0;
                        try
                        {
...
#region тестовое, картинки, без покрытия, штрихкода
            ...
            if (!isBorderDevice)
            {
                ...
                if (Devices.DeviceClass.IsXPS(card.device.DeviceType))
                {
                    ...
                    foreach (string printerName in printerNames)
                        cbXPSDrivers.Items.Add(printerName);
                    cbXPSDrivers.Text = ((Devices.PrinterClass)card.device).printerName;
                    da.To = 100;
                    da.Duration = TimeSpan.FromSeconds(1);
                    borderXPS.BeginAnimation(Border.HeightProperty, da);
                    da.To = 170;
                    da.Duration = TimeSpan.FromSeconds(1);
                    gbCard.BeginAnimation(GroupBox.HeightProperty, da);
                    isBorderDevice = true;
        ...
        private void cbDevice_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            switch (dev.ID)
            {
                case ((int)DeviceType.DC450):
                    card.device = new Devices.DC450();
                    break;
                case ((int)DeviceType.DC150):
                    card.device = new Devices.DC150();
                    break;
                case ((int)DeviceType.Simulator):
                    card.device = new Devices.Simulator();
                    break;
                case ((int)DeviceType.CD):
                    card.device = new Devices.XPSPrinter();
        ...
        private void bSmartPath_Click(object sender, RoutedEventArgs e)
        {
            if (currentObject == null || currentObject.OType != ObjectType.SmartField || ((SmartField)currentObject).SModule == null)
                return;
            OpenFileDialog oFile = new OpenFileDialog();
            oFile.Filter = $"{(string)this.FindResource("DllFiles")}|*.xml|{(string)this.FindResource("AllFiles")}|*.*";
            if (oFile.ShowDialog() == true)
            {
...
#region print
                    ...
                    if (Devices.DeviceClass.IsXPS(card.device.DeviceType))
                    {
                        for (int i = 0; i < card.objects.Count; i++)
                        {
                            if (card.objects[i].OType == ObjectType.MagStripe)
                            {
                                ((Devices.PrinterClass)card.device).SetMagstripe(((MagStripe)card.objects[i]).TextM);
                                break;
                            }
                        }
                    }
                    ...
                    if (DeviceClass.IsXPS(card.device.DeviceType))
                        ((Devices.XPSPrinter)card.device).SetImages(dw, null);
...
#warning smart timer elapsed not implemented
        ...
        private void cbXPSDrivers_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (!Devices.DeviceClass.IsXPS(card.device.DeviceType))
                return;
            if (cbXPSDrivers.SelectedIndex < 0)
                return;
            ((Devices.PrinterClass)card.device).printerName = cbXPSDrivers.SelectedValue.ToString();
        }
